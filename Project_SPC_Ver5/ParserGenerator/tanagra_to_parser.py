...
def printPaths(node):
    path = [0] * 1000
    path_list = []
    return generatePathsRecursively(node, path, 0, path_list)
...
def writeParser(parser_contents):
...
if __name__ == '__main__':
    ...
    for i in range(len(path_list)):
        parserElement = [1, 1, 0, 0, 0, -1, None, None]
        path = path_list[i]
        ...
        for j in range(len(path) - 1, -1, -1):
            ...
            if node1 is not None and node2 is not None:
                if node1.attr not in classes and node2.attr not in classes:
                    if (node1.attr, node2.attr) not in graphAttributePairs and \
                            (node1.attr not in used_attributes and node2.attr not in used_attributes):
                        graphAttributePairs.append((node1.attr, node2.attr))
                        used_attributes.append(node1.attr)
                        used_attributes.append(node2.attr)
                        graphNum += 1
        ...
        parserElement[6] = tree.orig_labels[graphAttributePairs[graphNum][0]]
        parserElement[7] = tree.orig_labels[graphAttributePairs[graphNum][1]]
        parserElements.append(parserElement.copy())
    writeParser(parserElements)
